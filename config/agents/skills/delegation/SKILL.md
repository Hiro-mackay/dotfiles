...
- Hardest design work (architecture with unclear shape, multi-system trade-offs) -> `fable`, only when I explicitly ask for it; that request overrides the delegation gate above. In plan mode, the harness's own Plan-agent phase counts as an explicit route
