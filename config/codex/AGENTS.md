...
- 3+ steps or architectural decisions: make the plan explicit before coding
...
- Use `$skill-name` when a reusable skill maps cleanly to the requested task
...
- The task references prior conversation decisions the subagent cannot see

When a subagent returns BLOCKED, read Progress / Blocker / Branches and handle
directly. Do not re-delegate the same ambiguity.
