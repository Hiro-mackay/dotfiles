...
実行ファイルのパスは端末固有なので、Gitのclean filterが追跡対象から除外する。
作業ツリーの `config.toml` には完全なローカル設定を残す。
Gitの比較とステージには、共有設定だけを抽出した内容を使う。
`setup-link.sh` がfilterを登録し、pre-commitは未登録端末の安全網として同じ抽出処理を使う。
共有する設定キーを増やす場合は、[設定抽出処理](../git/hooks/sanitize-codex-config.awk)の保持条件も更新する。
